# Global Revenue Brain — Oportunidades Verificadas

Gerado em: 2026-08-27T12:28:08.274215+00:00

## Resumo

- Total verificado: **1060**
- Fila prioritária: **49**
- rejected: **1007**
- actionable: **23**
- approval_required: **23**
- expired: **4**
- verified: **3**

## Fila prioritária

### 1. senddigidollar: integer-vs-decimal amount ambiguity is a 100× footgun

- Status: **actionable**
- Categoria: ai_integration
- Fonte: MCP Paid Opportunities
- URL: https://github.com/DigiByte-Core/digibyte/issues/424
- Recompensa: USD 10,000.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: alta
- Tempo estimado: 80.0 horas
- Risco: baixo
- Probabilidade estimada: 68.6%
- Score de verificação: 85.7/100
- KYC: não identificado
- Aprovação humana: não
- Forma de pagamento: GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: signed in to change notification settings Fork 91 Star 250 Code Issues 32 Pull requests 0 Discussions Actions Projects Wiki Security and quality 0 Insights Additional navigation options Code Issues Pull requests Discussions Actions Projects Wiki Security and quality Insights senddigidollar: integer-
- Próxima ação: **Preparar candidatura, proposta ou prova de conceito.**
- Motivo: Link ativo, recompensa explícita e baixo risco identificado.

### 2. [ Bounty $3k ] [ Research ] Collect and compare AI-generated AGI architecture proposals

- Status: **actionable**
- Categoria: open_source_bounty
- Fonte: GitHub Reward Issues
- URL: https://github.com/aLexzzz430/Cognitive-OS/issues/5
- Recompensa: USD 3,000.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 89.9%
- Score de verificação: 81.7/100
- KYC: não identificado
- Aprovação humana: não
- Forma de pagamento: GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: documented
- Próxima ação: **Preparar candidatura, proposta ou prova de conceito.**
- Motivo: Link ativo, recompensa explícita e baixo risco identificado.

### 3. [aLexzzz430/Cognitive-OS] [ Bounty $3k ] [ Research ] Collect and compare AI-generated

- Status: **actionable**
- Categoria: open_source_bounty
- Fonte: GitHub Reward Issues
- URL: https://github.com/Vikingr2023/awesome-agent-bounties/issues/167
- Recompensa: USD 3,000.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 89.9%
- Score de verificação: 81.7/100
- KYC: não identificado
- Aprovação humana: não
- Forma de pagamento: GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: documented
- Próxima ação: **Preparar candidatura, proposta ou prova de conceito.**
- Motivo: Link ativo, recompensa explícita e baixo risco identificado.

### 4. [aLexzzz430/Cognitive-OS] [ Bounty $3k ] [ Research ] Collect and compare AI-generated

- Status: **actionable**
- Categoria: open_source_bounty
- Fonte: GitHub Reward Issues
- URL: https://github.com/Vikingr2023/awesome-agent-bounties/issues/52
- Recompensa: USD 3,000.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 89.9%
- Score de verificação: 81.7/100
- KYC: não identificado
- Aprovação humana: não
- Forma de pagamento: GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: documented
- Próxima ação: **Preparar candidatura, proposta ou prova de conceito.**
- Motivo: Link ativo, recompensa explícita e baixo risco identificado.

### 5. Calculate the exact value of PI

- Status: **actionable**
- Categoria: open_source_bounty
- Fonte: GitHub Bounty Issues
- URL: https://github.com/xevrion-v2/agent-playground/issues/17
- Recompensa: USD 1,000.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 89.0%
- Score de verificação: 80.95/100
- KYC: não identificado
- Aprovação humana: não
- Forma de pagamento: GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: signed in to change notification settings Fork 440 Star 260 Code Issues 1
- Próxima ação: **Preparar candidatura, proposta ou prova de conceito.**
- Motivo: Link ativo, recompensa explícita e baixo risco identificado.

### 6. Approval: create Stripe Payment Links for paid audits

- Status: **actionable**
- Categoria: ai_integration
- Fonte: MCP Paid Opportunities
- URL: https://github.com/Davidleeops/mcpscan/issues/3
- Recompensa: USD 3,500.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 88.6%
- Score de verificação: 80.55/100
- KYC: não identificado
- Aprovação humana: não
- Forma de pagamento: Stripe, GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: signed in to change notification settings Fork 0 Star 0 Code Issues 5 Pull requests 1 Actions Projects Security and quality 0 Insights Additional navigation options Code Issues Pull requests Actions Projects Security and quality Insights Approval: create Stripe Payment Links for paid audits #3 New i
- Próxima ação: **Preparar candidatura, proposta ou prova de conceito.**
- Motivo: Link ativo, recompensa explícita e baixo risco identificado.

### 7. Fixed-Goal Grant Proposal

- Status: **actionable**
- Categoria: grant
- Fonte: GitHub Grant Issues
- URL: https://github.com/FreeCAD/FPA-grant-proposals/issues/104
- Recompensa: EUR 5,000.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 87.1%
- Score de verificação: 79.21/100
- KYC: não identificado
- Aprovação humana: não
- Forma de pagamento: GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: for a performant dynamic LOD system will need to be established during the prototype stage
- Próxima ação: **Preparar candidatura, proposta ou prova de conceito.**
- Motivo: Link ativo, recompensa explícita e baixo risco identificado.

### 8. [BOUNTY $100] 🐜The Memanto Bug & Exploit Challenge

- Status: **actionable**
- Categoria: ai_bounty
- Fonte: AI Agent Bounties
- URL: https://github.com/moorcheh-ai/memanto/issues/770
- Recompensa: USD 100.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: média
- Tempo estimado: 30.0 horas
- Risco: baixo
- Probabilidade estimada: 76.8%
- Score de verificação: 76.85/100
- KYC: não identificado
- Aprovação humana: não
- Forma de pagamento: GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: Pull Requests
- Próxima ação: **Preparar candidatura, proposta ou prova de conceito.**
- Motivo: Link ativo, recompensa explícita e baixo risco identificado.

### 9. Kendraio App MusicBrainz integration (bounty)

- Status: **actionable**
- Categoria: open_source_bounty
- Fonte: GitHub Bounty Issues
- URL: https://github.com/kendraio/kendraio-app/issues/184
- Recompensa: USD 500.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 84.1%
- Score de verificação: 76.45/100
- KYC: não identificado
- Aprovação humana: não
- Forma de pagamento: GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: Candidates should have some existing experience with JavaScript and web APIs
- Próxima ação: **Preparar candidatura, proposta ou prova de conceito.**
- Motivo: Link ativo, recompensa explícita e baixo risco identificado.

### 10. [BOUNTY $100] 🐜The Memanto Bug & Exploit Challenge

- Status: **actionable**
- Categoria: open_source_bounty
- Fonte: GitHub Paid Issues
- URL: https://github.com/moorcheh-ai/memanto/issues/770
- Recompensa: USD 100.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: média
- Tempo estimado: 30.0 horas
- Risco: baixo
- Probabilidade estimada: 75.9%
- Score de verificação: 75.9/100
- KYC: não identificado
- Aprovação humana: não
- Forma de pagamento: GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: Pull Requests
- Próxima ação: **Preparar candidatura, proposta ou prova de conceito.**
- Motivo: Link ativo, recompensa explícita e baixo risco identificado.

### 11. 🎯 Bounty Alert: 31 New Opportunities — 2026-07-15 03:21 UTC

- Status: **actionable**
- Categoria: ai_bounty
- Fonte: AI Agent Bounties
- URL: https://github.com/greyw0rks/bountyscout/issues/265
- Recompensa: USD 150.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 83.0%
- Score de verificação: 75.45/100
- KYC: não identificado
- Aprovação humana: não
- Forma de pagamento: GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: signed in to change notification settings Fork 8 Star 7 Code Issues 264 Pull requests 4 Actions Projects Security and quality 0 Insights Additional navigation options Code Issues Pull requests Actions Projects Security and quality Insights 🎯 Bounty Alert: 31 New Opportunities — 2026-07-15 03:21 UTC
- Próxima ação: **Preparar candidatura, proposta ou prova de conceito.**
- Motivo: Link ativo, recompensa explícita e baixo risco identificado.

### 12. Support actual payment amount confirmation and rate-shift tracking for foreign scheduled expenses

- Status: **actionable**
- Categoria: paid_development
- Fonte: GitHub Paid Issues
- URL: https://github.com/pedrazamiguez/split-trip/issues/1400
- Recompensa: EUR 130.74
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 82.7%
- Score de verificação: 75.17/100
- KYC: não identificado
- Aprovação humana: não
- Forma de pagamento: GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: properly formatted using existing locale-aware formatting utilities (e
- Próxima ação: **Preparar candidatura, proposta ou prova de conceito.**
- Motivo: Link ativo, recompensa explícita e baixo risco identificado.

### 13. monte_carlo_layer_means default seed makes a small-n draw an exact prefix of a large-n draw, and the compare_against_monte_carlo MSE column is easy to misread

- Status: **actionable**
- Categoria: competition
- Fonte: GitHub Prize Issues
- URL: https://github.com/AIcrowd/whest-starterkit/issues/39
- Recompensa: USD 500.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 82.2%
- Score de verificação: 74.69/100
- KYC: não identificado
- Aprovação humana: não
- Forma de pagamento: GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: signed in to change notification settings Fork 14 Star 9 Code Issues 4 Pull requests 0 Actions Projects Security and quality 0 Insights Additional navigation options Code Issues Pull requests Actions Projects Security and quality Insights monte_carlo_layer_means default seed makes a small-n draw an
- Próxima ação: **Preparar candidatura, proposta ou prova de conceito.**
- Motivo: Link ativo, recompensa explícita e baixo risco identificado.

### 14. [BOUNTY $200] 🐜 The Great Memory Migration: Own Your Agentic Memory with Memanto + OKF

- Status: **actionable**
- Categoria: competition
- Fonte: GitHub Prize Issues
- URL: https://github.com/moorcheh-ai/memanto/issues/1609
- Recompensa: USD 200.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 81.8%
- Score de verificação: 74.35/100
- KYC: não identificado
- Aprovação humana: não
- Forma de pagamento: GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: signed in to change notification settings Fork 631 Star 1
- Próxima ação: **Preparar candidatura, proposta ou prova de conceito.**
- Motivo: Link ativo, recompensa explícita e baixo risco identificado.

### 15. 🎯 Bounty Alert: 30 New Opportunities — 2026-07-14 10:46 UTC

- Status: **actionable**
- Categoria: ai_bounty
- Fonte: AI Agent Bounties
- URL: https://github.com/greyw0rks/bountyscout/issues/256
- Recompensa: USD 28.40
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 78.6%
- Score de verificação: 71.45/100
- KYC: não identificado
- Aprovação humana: não
- Forma de pagamento: GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: signed in to change notification settings Fork 8 Star 7 Code Issues 264 Pull requests 4 Actions Projects Security and quality 0 Insights Additional navigation options Code Issues Pull requests Actions Projects Security and quality Insights 🎯 Bounty Alert: 30 New Opportunities — 2026-07-14 10:46 UTC
- Próxima ação: **Preparar candidatura, proposta ou prova de conceito.**
- Motivo: Link ativo, recompensa explícita e baixo risco identificado.

### 16. [AGENT-TASK] Content-generation agent — auto-posts from bounty outcomes

- Status: **actionable**
- Categoria: ai_bounty
- Fonte: AI Agent Bounties
- URL: https://github.com/Nexussyn/ai-growth-engine/issues/5
- Recompensa: USD 5.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 78.6%
- Score de verificação: 71.45/100
- KYC: não identificado
- Aprovação humana: não
- Forma de pagamento: USDC, GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: signed in to change notification settings Fork 35 Star 6 Code Issues 5 Pull requests 71 Actions Projects Security and quality 0 Insights Additional navigation options Code Issues Pull requests Actions Projects Security and quality Insights [AGENT-TASK] Content-generation agent — auto-posts from boun
- Próxima ação: **Preparar candidatura, proposta ou prova de conceito.**
- Motivo: Link ativo, recompensa explícita e baixo risco identificado.

### 17. bounty: $15 USDC — add-only docs/BAZAAR_AUDIT.md

- Status: **actionable**
- Categoria: ai_integration
- Fonte: MCP Paid Opportunities
- URL: https://github.com/kwizzlesurp10-ctrl/x402-mcp/issues/495
- Recompensa: USD 15.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 78.6%
- Score de verificação: 71.45/100
- KYC: não identificado
- Aprovação humana: não
- Forma de pagamento: USDC, GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: signed in to change notification settings Fork 0 Star 1 Code Issues 33 Pull requests 0 Actions Projects Security and quality 0 Insights Additional navigation options Code Issues Pull requests Actions Projects Security and quality Insights bounty: $15 USDC — add-only docs/BAZAAR_AUDIT
- Próxima ação: **Preparar candidatura, proposta ou prova de conceito.**
- Motivo: Link ativo, recompensa explícita e baixo risco identificado.

### 18. bounty: $5 USDC — add-only docs/TOOL_LEGIBILITY.md

- Status: **actionable**
- Categoria: ai_integration
- Fonte: MCP Paid Opportunities
- URL: https://github.com/kwizzlesurp10-ctrl/x402-mcp/issues/494
- Recompensa: USD 5.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 78.6%
- Score de verificação: 71.45/100
- KYC: não identificado
- Aprovação humana: não
- Forma de pagamento: USDC, GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: signed in to change notification settings Fork 0 Star 1 Code Issues 33 Pull requests 0 Actions Projects Security and quality 0 Insights Additional navigation options Code Issues Pull requests Actions Projects Security and quality Insights bounty: $5 USDC — add-only docs/TOOL_LEGIBILITY
- Próxima ação: **Preparar candidatura, proposta ou prova de conceito.**
- Motivo: Link ativo, recompensa explícita e baixo risco identificado.

### 19. [Bug] fix.py is a stale hardcoded stub

- Status: **actionable**
- Categoria: open_source_bounty
- Fonte: GitHub Bounty Issues
- URL: https://github.com/Hazyshades/Sendly-Test-Repo/issues/82
- Recompensa: USD 10.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 77.7%
- Score de verificação: 70.64/100
- KYC: não identificado
- Aprovação humana: não
- Forma de pagamento: USDC, GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: signed in to change notification settings Fork 41 Star 1 Code Issues 13 Pull requests 67 Actions Projects Security and quality 0 Insights Additional navigation options Code Issues Pull requests Actions Projects Security and quality Insights [Bug] fix
- Próxima ação: **Preparar candidatura, proposta ou prova de conceito.**
- Motivo: Link ativo, recompensa explícita e baixo risco identificado.

### 20. Ops dashboard

- Status: **actionable**
- Categoria: ai_integration
- Fonte: MCP Paid Opportunities
- URL: https://github.com/ModelMirrorAI/fedcourtsai/issues/630
- Recompensa: USD 55.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 77.1%
- Score de verificação: 70.1/100
- KYC: não identificado
- Aprovação humana: não
- Forma de pagamento: GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: signed in to change notification settings Fork 0 Star 1 Code Issues 14 Pull requests 1 Actions Security and quality 0 Insights Additional navigation options Code Issues Pull requests Actions Security and quality Insights Ops dashboard #630 New issue Copy link New issue Copy link Open Open Ops dashbo
- Próxima ação: **Preparar candidatura, proposta ou prova de conceito.**
- Motivo: Link ativo, recompensa explícita e baixo risco identificado.

### 21. [BOUNTY $80] Notifications — 统一通知中心 (Gotify + Apprise)

- Status: **actionable**
- Categoria: open_source_bounty
- Fonte: GitHub Bounty Issues
- URL: https://github.com/illbnm/homelab-stack/issues/13
- Recompensa: USD 80.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 77.1%
- Score de verificação: 70.06/100
- KYC: não identificado
- Aprovação humana: não
- Forma de pagamento: USDT, GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: signed in to change notification settings Fork 112 Star 9 Code Issues 18 Pull requests 480 Actions Projects Security and quality 0 Insights Additional navigation options Code Issues Pull requests Actions Projects Security and quality Insights [BOUNTY $80] Notifications — 统一通知中心 (Gotify + Apprise) #1
- Próxima ação: **Preparar candidatura, proposta ou prova de conceito.**
- Motivo: Link ativo, recompensa explícita e baixo risco identificado.

### 22. Automatic authoritative external-billing Subscription adoption (CODEX...

- Status: **actionable**
- Categoria: paid_development
- Fonte: GitHub Paid Issues
- URL: https://github.com/jaywedgeworth22/api-usage-monitor/issues/285
- Recompensa: USD 6.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 76.8%
- Score de verificação: 69.83/100
- KYC: não identificado
- Aprovação humana: não
- Forma de pagamento: GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: signed in to change notification settings Fork 0 Star 1 Code Issues 123 Pull requests 0 Actions Projects Security and quality 0 Insights Additional navigation options Code Issues Pull requests Actions Projects Security and quality Insights Automatic authoritative external-billing Subscription adopti
- Próxima ação: **Preparar candidatura, proposta ou prova de conceito.**
- Motivo: Link ativo, recompensa explícita e baixo risco identificado.

### 23. [Bounty: 3 ETH] implement Goose class in SuperCollider

- Status: **actionable**
- Categoria: open_source_bounty
- Fonte: GitHub Bounty Issues
- URL: https://github.com/dwebagents/AgentPipe/issues/131
- Recompensa: ETH 3.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 75.2%
- Score de verificação: 68.38/100
- KYC: não identificado
- Aprovação humana: não
- Forma de pagamento: GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: implemented in the SuperCollider language: - the **honk** method should synthesize the sound of 74 geese honking
- Próxima ação: **Preparar candidatura, proposta ou prova de conceito.**
- Motivo: Link ativo, recompensa explícita e baixo risco identificado.

### 24. Grant Application - Educating Communities by Educating Their Leaders

- Status: **approval_required**
- Categoria: grant
- Fonte: GitHub Grant Issues
- URL: https://github.com/ZcashCommunityGrants/zcashcommunitygrants/issues/356
- Recompensa: USD 50,000.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: alta
- Tempo estimado: 80.0 horas
- Risco: baixo
- Probabilidade estimada: 68.6%
- Score de verificação: 85.7/100
- KYC: sim
- Aprovação humana: sim
- Forma de pagamento: GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: signed in to change notification settings Fork 9 Star 14 Code Issues 42 Pull requests 0 Actions Projects Security and quality 0 Insights Additional navigation options Code Issues Pull requests Actions Projects Security and quality Insights Grant Application - Educating Communities by Educating Their
- Próxima ação: **Revisar requisitos e autorizar a preparação.**
- Motivo: A oportunidade é promissora, mas exige ação humana sensível.

### 25. DeXe Protocol

- Status: **approval_required**
- Categoria: authorized_bug_bounty
- Fonte: Immunefi
- URL: https://immunefi.com/bug-bounty/dexeprotocol/information/
- Recompensa: USD 500,000.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: alta
- Tempo estimado: 80.0 horas
- Risco: baixo
- Probabilidade estimada: 67.8%
- Score de verificação: 84.75/100
- KYC: sim
- Aprovação humana: sim
- Forma de pagamento: USDC
- Restrições geográficas: Eligibility: Authorized program; inspect scope before testing
- Requisitos: Authorized program; inspect scope before testing
- Próxima ação: **Revisar requisitos e autorizar a preparação.**
- Motivo: A oportunidade é promissora, mas exige ação humana sensível.

### 26. StackingDAO

- Status: **approval_required**
- Categoria: authorized_bug_bounty
- Fonte: Immunefi
- URL: https://immunefi.com/bug-bounty/stackingdao/information/
- Recompensa: USD 100,000.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: alta
- Tempo estimado: 80.0 horas
- Risco: baixo
- Probabilidade estimada: 67.8%
- Score de verificação: 84.75/100
- KYC: sim
- Aprovação humana: sim
- Forma de pagamento: USDC
- Restrições geográficas: Eligibility: Authorized program; inspect scope before testing
- Requisitos: Authorized program; inspect scope before testing
- Próxima ação: **Revisar requisitos e autorizar a preparação.**
- Motivo: A oportunidade é promissora, mas exige ação humana sensível.

### 27. Immunefi

- Status: **approval_required**
- Categoria: authorized_bug_bounty
- Fonte: Immunefi
- URL: https://immunefi.com/bug-bounty/immunefi/information/
- Recompensa: USD 50,000.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: alta
- Tempo estimado: 80.0 horas
- Risco: baixo
- Probabilidade estimada: 67.8%
- Score de verificação: 84.75/100
- KYC: sim
- Aprovação humana: sim
- Forma de pagamento: USDC
- Restrições geográficas: Eligibility: Authorized program; inspect scope before testing
- Requisitos: Authorized program; inspect scope before testing
- Próxima ação: **Revisar requisitos e autorizar a preparação.**
- Motivo: A oportunidade é promissora, mas exige ação humana sensível.

### 28. Hedera

- Status: **approval_required**
- Categoria: authorized_bug_bounty
- Fonte: Immunefi
- URL: https://immunefi.com/bug-bounty/hedera/information/
- Recompensa: USD 14,500.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: alta
- Tempo estimado: 80.0 horas
- Risco: baixo
- Probabilidade estimada: 67.8%
- Score de verificação: 84.75/100
- KYC: sim
- Aprovação humana: sim
- Forma de pagamento: USDC
- Restrições geográficas: Eligibility: Authorized program; inspect scope before testing
- Requisitos: Authorized program; inspect scope before testing
- Próxima ação: **Revisar requisitos e autorizar a preparação.**
- Motivo: A oportunidade é promissora, mas exige ação humana sensível.

### 29. Data Analytics Bounty

- Status: **approval_required**
- Categoria: crypto_bounty
- Fonte: Crypto Bounties
- URL: https://github.com/Honest-Protocol/Gitcoin_Metamorphosis/issues/3
- Recompensa: USD 2,250.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 89.9%
- Score de verificação: 81.7/100
- KYC: não identificado
- Aprovação humana: sim
- Forma de pagamento: USDC, GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: Data analytics can be based on any tool the participant sees fit to utilize
- Próxima ação: **Revisar requisitos e autorizar a preparação.**
- Motivo: A oportunidade é promissora, mas exige ação humana sensível.

### 30. Tool discovery 2026-07-15: 6 candidates

- Status: **approval_required**
- Categoria: paid_development
- Fonte: GitHub Paid Issues
- URL: https://github.com/TarronKayAUA/aua-ai-hub/issues/21
- Recompensa: USD 4,167.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: média
- Tempo estimado: 30.0 horas
- Risco: baixo
- Probabilidade estimada: 80.1%
- Score de verificação: 80.14/100
- KYC: não identificado
- Aprovação humana: sim
- Forma de pagamento: GitHub Sponsors
- Restrições geográficas: Open Tool discovery 2026-07-15: 6 candidates #21 Copy link Description TarronKayAUA opened on Jul 15, 2026 Issue body actions This is the monthly propose-only tool dis
- Requisitos: signed in to change notification settings Fork 0 Star 0 Code Issues 1 Pull requests 0 Discussions Actions Projects Security and quality 0 Insights Additional navigation options Code Issues Pull requests Discussions Actions Projects Security and quality Insights Tool discovery 2026-07-15: 6 candidate
- Próxima ação: **Revisar requisitos e autorizar a preparação.**
- Motivo: A oportunidade é promissora, mas exige ação humana sensível.

### 31. [BOUNTY $200] Testing — 自动化测试套件

- Status: **approval_required**
- Categoria: open_source_bounty
- Fonte: GitHub Bounty Issues
- URL: https://github.com/illbnm/homelab-stack/issues/14
- Recompensa: USD 280.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 83.7%
- Score de verificação: 76.05/100
- KYC: não identificado
- Aprovação humana: sim
- Forma de pagamento: USDT, GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: signed in to change notification settings Fork 112 Star 9 Code Issues 18 Pull requests 480 Actions Projects Security and quality 0 Insights Additional navigation options Code Issues Pull requests Actions Projects Security and quality Insights [BOUNTY $200] Testing — 自动化测试套件 #14 New issue Copy link N
- Próxima ação: **Revisar requisitos e autorizar a preparação.**
- Motivo: A oportunidade é promissora, mas exige ação humana sensível.

### 32. Zest Protocol V2

- Status: **approval_required**
- Categoria: authorized_bug_bounty
- Fonte: Immunefi
- URL: https://immunefi.com/bug-bounty/zest-protocol-v2/information/
- Recompensa: USD 150.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: média
- Tempo estimado: 30.0 horas
- Risco: baixo
- Probabilidade estimada: 75.7%
- Score de verificação: 75.75/100
- KYC: sim
- Aprovação humana: sim
- Forma de pagamento: USDC
- Restrições geográficas: Eligibility: Authorized program; inspect scope before testing
- Requisitos: Authorized program; inspect scope before testing
- Próxima ação: **Revisar requisitos e autorizar a preparação.**
- Motivo: A oportunidade é promissora, mas exige ação humana sensível.

### 33. [0.10 USDC autonomous bounty] Complete an agent-skill claim-to-payout canary

- Status: **approval_required**
- Categoria: ai_integration
- Fonte: MCP Paid Opportunities
- URL: https://github.com/NSPG13/agent-bounties/issues/277
- Recompensa: USD 0.11
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 79.0%
- Score de verificação: 71.85/100
- KYC: não identificado
- Aprovação humana: sim
- Forma de pagamento: USDC, GitHub Sponsors
- Restrições geográficas: eligibility 1
- Requisitos: A wallet different from the creator owns the active canonical claim round
- Próxima ação: **Revisar requisitos e autorizar a preparação.**
- Motivo: A oportunidade é promissora, mas exige ação humana sensível.

### 34. [0.10 USDC autonomous bounty] Complete an MCP-discovered claim-to-payout canary

- Status: **approval_required**
- Categoria: ai_integration
- Fonte: MCP Paid Opportunities
- URL: https://github.com/NSPG13/agent-bounties/issues/276
- Recompensa: USD 0.11
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 79.0%
- Score de verificação: 71.85/100
- KYC: não identificado
- Aprovação humana: sim
- Forma de pagamento: USDC, GitHub Sponsors
- Restrições geográficas: eligibility 1
- Requisitos: A wallet different from the creator owns the active canonical claim round
- Próxima ação: **Revisar requisitos e autorizar a preparação.**
- Motivo: A oportunidade é promissora, mas exige ação humana sensível.

### 35. [0.10 USDC autonomous bounty] Complete a wallet claim-to-payout canary

- Status: **approval_required**
- Categoria: ai_integration
- Fonte: MCP Paid Opportunities
- URL: https://github.com/NSPG13/agent-bounties/issues/275
- Recompensa: USD 0.11
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 79.0%
- Score de verificação: 71.85/100
- KYC: não identificado
- Aprovação humana: sim
- Forma de pagamento: USDC, GitHub Sponsors
- Restrições geográficas: eligibility 1
- Requisitos: A wallet different from the creator owns the active canonical claim round
- Próxima ação: **Revisar requisitos e autorizar a preparação.**
- Motivo: A oportunidade é promissora, mas exige ação humana sensível.

### 36. [0.10 USDC autonomous meta-bounty] Create a paid coding-bounty loop

- Status: **approval_required**
- Categoria: open_source_bounty
- Fonte: GitHub Bounty Issues
- URL: https://github.com/NSPG13/agent-bounties/issues/274
- Recompensa: USD 0.11
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 79.0%
- Score de verificação: 71.85/100
- KYC: não identificado
- Aprovação humana: sim
- Forma de pagamento: USDC, GitHub Sponsors
- Restrições geográficas: eligibility 1
- Requisitos: so the canonical parent verifier passes and emits BountySettled; the child solver must be a different wallet from the parent solver
- Próxima ação: **Revisar requisitos e autorizar a preparação.**
- Motivo: A oportunidade é promissora, mas exige ação humana sensível.

### 37. [0.10 USDC autonomous meta-bounty] Post a funded agent-tooling bounty completed by another wallet

- Status: **approval_required**
- Categoria: open_source_bounty
- Fonte: GitHub Bounty Issues
- URL: https://github.com/NSPG13/agent-bounties/issues/273
- Recompensa: USD 0.11
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 79.0%
- Score de verificação: 71.85/100
- KYC: não identificado
- Aprovação humana: sim
- Forma de pagamento: USDC, GitHub Sponsors
- Restrições geográficas: eligibility 1
- Requisitos: so the canonical parent verifier passes and emits BountySettled; the child solver must be a different wallet from the parent solver
- Próxima ação: **Revisar requisitos e autorizar a preparação.**
- Motivo: A oportunidade é promissora, mas exige ação humana sensível.

### 38. [UNFUNDED: 0/2.01 USDC] Complete a machine-API paid loop

- Status: **approval_required**
- Categoria: ai_integration
- Fonte: MCP Paid Opportunities
- URL: https://github.com/NSPG13/agent-bounties/issues/249
- Recompensa: USD 2.01
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 78.9%
- Score de verificação: 71.7/100
- KYC: não identificado
- Aprovação humana: sim
- Forma de pagamento: USDC, GitHub Sponsors
- Restrições geográficas: eligibility and payment
- Requisitos: and payment
- Próxima ação: **Revisar requisitos e autorizar a preparação.**
- Motivo: A oportunidade é promissora, mas exige ação humana sensível.

### 39. [2 USDC autonomous bounty] Complete an independent-relayer paid loop

- Status: **approval_required**
- Categoria: ai_integration
- Fonte: MCP Paid Opportunities
- URL: https://github.com/NSPG13/agent-bounties/issues/250
- Recompensa: USD 2.01
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 78.9%
- Score de verificação: 71.7/100
- KYC: não identificado
- Aprovação humana: sim
- Forma de pagamento: USDC, GitHub Sponsors
- Restrições geográficas: eligibility and payment
- Requisitos: and payment
- Próxima ação: **Revisar requisitos e autorizar a preparação.**
- Motivo: A oportunidade é promissora, mas exige ação humana sensível.

### 40. [2 USDC autonomous bounty] Complete a browser-wallet paid loop

- Status: **approval_required**
- Categoria: ai_integration
- Fonte: MCP Paid Opportunities
- URL: https://github.com/NSPG13/agent-bounties/issues/248
- Recompensa: USD 2.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 78.9%
- Score de verificação: 71.7/100
- KYC: não identificado
- Aprovação humana: sim
- Forma de pagamento: USDC, GitHub Sponsors
- Restrições geográficas: eligibility and payment
- Requisitos: and payment
- Próxima ação: **Revisar requisitos e autorizar a preparação.**
- Motivo: A oportunidade é promissora, mas exige ação humana sensível.

### 41. P1 — First verified revenue: USD 10+

- Status: **approval_required**
- Categoria: ai_bounty
- Fonte: AI Agent Bounties
- URL: https://github.com/jadeldiaz01-png/AUREUS-2026-Autonomous-Universal-Revenue-Efficient-Utility-Scheduler/issues/2
- Recompensa: USD 10.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 78.9%
- Score de verificação: 71.7/100
- KYC: sim
- Aprovação humana: sim
- Forma de pagamento: GitHub Sponsors
- Restrições geográficas: eligibility/onboarding is complete
- Requisitos: signed in to change notification settings Fork 0 Star 0 Code Issues 1 Pull requests 0 Actions Projects Security and quality 0 Insights Additional navigation options Code Issues Pull requests Actions Projects Security and quality Insights P1 — First verified revenue: USD 10+ #2 New issue Copy link Ne
- Próxima ação: **Revisar requisitos e autorizar a preparação.**
- Motivo: A oportunidade é promissora, mas exige ação humana sensível.

### 42. [Bounty] [3 USDC][Open Competition V2] Map the shortest discovery path for eight AI-agent

- Status: **approval_required**
- Categoria: ai_bounty
- Fonte: AI Agent Bounties
- URL: https://github.com/zhangjiayang6835-cyber/bounty-plaza/issues/834
- Recompensa: USD 3.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 78.8%
- Score de verificação: 71.6/100
- KYC: não identificado
- Aprovação humana: sim
- Forma de pagamento: USDC, GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: signed in to change notification settings Fork 44 Star 7 Code Issues 405 Pull requests 218 Actions Security and quality 0 Insights Additional navigation options Code Issues Pull requests Actions Security and quality Insights [Bounty] [3 USDC][Open Competition V2] Map the shortest discovery path for
- Próxima ação: **Revisar requisitos e autorizar a preparação.**
- Motivo: A oportunidade é promissora, mas exige ação humana sensível.

### 43. bounty: $25 USDC — add-only docs/LIVE_LEDGER.md

- Status: **approval_required**
- Categoria: ai_integration
- Fonte: MCP Paid Opportunities
- URL: https://github.com/kwizzlesurp10-ctrl/x402-mcp/issues/496
- Recompensa: USD 25.00
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 78.6%
- Score de verificação: 71.45/100
- KYC: não identificado
- Aprovação humana: sim
- Forma de pagamento: USDC, GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: signed in to change notification settings Fork 0 Star 1 Code Issues 33 Pull requests 0 Actions Projects Security and quality 0 Insights Additional navigation options Code Issues Pull requests Actions Projects Security and quality Insights bounty: $25 USDC — add-only docs/LIVE_LEDGER
- Próxima ação: **Revisar requisitos e autorizar a preparação.**
- Motivo: A oportunidade é promissora, mas exige ação humana sensível.

### 44. [UNFUNDED: 0/0.11 USDC] Complete a fresh earning-loop friction canary

- Status: **approval_required**
- Categoria: open_source_bounty
- Fonte: GitHub Paid Issues
- URL: https://github.com/NSPG13/agent-bounties/issues/291
- Recompensa: USD 0.10
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: baixo
- Probabilidade estimada: 78.0%
- Score de verificação: 70.9/100
- KYC: não identificado
- Aprovação humana: sim
- Forma de pagamento: USDC, GitHub Sponsors
- Restrições geográficas: eligibility 1
- Requisitos: A wallet different from the creator owns the active canonical claim round
- Próxima ação: **Revisar requisitos e autorizar a preparação.**
- Motivo: A oportunidade é promissora, mas exige ação humana sensível.

### 45. [gig] DeskCrew Bounty Board

- Status: **approval_required**
- Categoria: ai_bounty
- Fonte: AI Agent Bounties
- URL: https://github.com/gigs-sh/gigs-sh/issues/5
- Recompensa: não identificada
- Prazo: não identificado
- Capital necessário: sim
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: alto
- Probabilidade estimada: 2.0%
- Score de verificação: 0.0/100
- KYC: sim
- Aprovação humana: sim
- Forma de pagamento: USDC, GitHub Sponsors
- Restrições geográficas: open to anonymous AI agents
- Requisitos: signed in to change notification settings Fork 1 Star 1 Code Issues 5 Pull requests 0 Actions Projects Security and quality 0 Insights Additional navigation options Code Issues Pull requests Actions Projects Security and quality Insights [gig] DeskCrew Bounty Board #5 New issue Copy link New issue C
- Próxima ação: **Solicitar aprovação humana antes de qualquer gasto.**
- Motivo: A oportunidade aparenta exigir capital, taxa, depósito ou compra.

### 46. rwa-adapter::deposit() never transfers tokens into the vault — the entire yield-vault custody model is a no-op

- Status: **approval_required**
- Categoria: competition
- Fonte: GitHub Prize Issues
- URL: https://github.com/INVERSEARENA/inversearena-frontend/issues/1299
- Recompensa: não identificada
- Prazo: não identificado
- Capital necessário: sim
- Dificuldade: média
- Tempo estimado: 30.0 horas
- Risco: alto
- Probabilidade estimada: 2.0%
- Score de verificação: 0.0/100
- KYC: não identificado
- Aprovação humana: sim
- Forma de pagamento: GitHub Sponsors
- Restrições geográficas: não identificadas
- Requisitos: signed in to change notification settings Fork 113 Star 3 Code Issues 63 Pull requests 0 Actions Projects Security and quality 0 Insights Additional navigation options Code Issues Pull requests Actions Projects Security and quality Insights rwa-adapter::deposit() never transfers tokens into the vaul
- Próxima ação: **Solicitar aprovação humana antes de qualquer gasto.**
- Motivo: A oportunidade aparenta exigir capital, taxa, depósito ou compra.

### 47. Freelance Copywriter

- Status: **verified**
- Categoria: paid_development
- Fonte: Remotive
- URL: https://remotive.com/remote-jobs/writing/freelance-copywriter-1749306
- Recompensa: não identificada
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: médio
- Probabilidade estimada: 31.8%
- Score de verificação: 28.9/100
- KYC: não identificado
- Aprovação humana: não
- Forma de pagamento: não identificada
- Restrições geográficas: não identificadas
- Requisitos: não identificados
- Próxima ação: **Realizar revisão humana curta antes da preparação.**
- Motivo: A oportunidade parece executável, mas ainda possui informação incompleta.

### 48. Schedule a call

- Status: **verified**
- Categoria: coding_bounty
- Fonte: Algora Open Bounties
- URL: https://cal.com/ioannisflo
- Recompensa: não identificada
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: média
- Tempo estimado: 30.0 horas
- Risco: médio
- Probabilidade estimada: 27.8%
- Score de verificação: 27.82/100
- KYC: não identificado
- Aprovação humana: não
- Forma de pagamento: não identificada
- Restrições geográficas: não identificadas
- Requisitos: não identificados
- Próxima ação: **Realizar revisão humana curta antes da preparação.**
- Motivo: A oportunidade parece executável, mas ainda possui informação incompleta.

### 49. Preliminary agenda announced for the Open Technology Research Symposium 2026

- Status: **verified**
- Categoria: open_source
- Fonte: Open Source Initiative
- URL: https://opensource.org/blog/preliminary-agenda-announced-for-the-open-technology-research-symposium-2026
- Recompensa: não identificada
- Prazo: não identificado
- Capital necessário: não identificado
- Dificuldade: baixa
- Tempo estimado: 8.0 horas
- Risco: médio
- Probabilidade estimada: 30.3%
- Score de verificação: 27.54/100
- KYC: não identificado
- Aprovação humana: não
- Forma de pagamento: não identificada
- Restrições geográficas: não identificadas
- Requisitos: for Software Open Source AI Open Source AI OSAI Definition Process Timeline Open Weights FAQ Checklist Community Become an Individual Member Become an OSI Affiliate Affiliate Organizations Maintainers Events Forum OpenSource
- Próxima ação: **Realizar revisão humana curta antes da preparação.**
- Motivo: A oportunidade parece executável, mas ainda possui informação incompleta.
